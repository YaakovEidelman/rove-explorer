...
public class CompressTests
{
    ...
    private static string[] TarNames(string archivePath)
    {
    ...
    [Fact]
    public void OneFolderBecomesATarGzNamedAfterIt()
    {
        using TempDir tmp = new();
        tmp.Dir("project");
        tmp.Dir("project/src");
        File.WriteAllText(tmp.Sub("project", "src", "main.cs"), "code");

        CommandResult<OpResult[]> result = _actions.CompressItems(
            new([tmp.Sub("project")], tmp.Path, ArchiveFormat.TarGz));

        Assert.True(result.IsOk, result.Message);
        Assert.Equal(tmp.Sub("project.tar.gz"), result.Data![0].Item!.FullPath);
        Assert.Contains("project/src/main.cs", TarNames(tmp.Sub("project.tar.gz")));
    }

    [Fact]
    public void ATarGzRoveMadeIsATarGzRoveCanUnpack()
    {
        using TempDir tmp = new();
        tmp.Dir("project");
        File.WriteAllText(tmp.Sub("project", "main.cs"), "code");

        _actions.CompressItems(new([tmp.Sub("project")], tmp.Path, ArchiveFormat.TarGz));
        string away = tmp.Dir("elsewhere");
        CommandResult<OpResult[]> unpacked = _actions.ExtractArchives(new([tmp.Sub("project.tar.gz")], away));

        Assert.True(unpacked.IsOk, unpacked.Message);
        Assert.Equal("code", File.ReadAllText(Path.Combine(away, "project", "project", "main.cs")));
    }
}
