...
public partial class Actions
{
    public CommandResult<string?> CopyOutOfArchive(CopyOutOfArchiveArgs args)
    {
        if (!ArchivePath.TryParse(args.Path, out ArchivePath inside) || inside.IsRoot)
            return CommandResult<string?>.Fail("not_in_archive", $"{args.Path} is not inside an archive.");
        ...
        {
            return CommandResult<string?>.Fail("not_found", $"{inside.Name} is not in that archive any more.");
        }
        catch (InvalidDataException)
        {
            return CommandResult<string?>.Fail(
                "bad_archive", $"{Path.GetFileName(inside.Archive)} is damaged, or is not really an archive.");
        }
    ...
    public CommandResult<string?> LaunchFile(LaunchFileArgs args)
    {
        ...
        {
            return CommandResult<string?>.Fail(
                "in_archive", "A file inside an archive has to be taken out of it before anything can open it.");
        }
    ...
    public CommandResult<string?> OpenTerminal(OpenTerminalArgs args)
    {
        ...
        {
            return CommandResult<string?>.Fail(
                "in_archive", "A terminal cannot open inside an archive. Extract it first.");
        }
