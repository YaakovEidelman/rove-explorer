...
public partial class Actions
{
    ...
    )
    {
        ...
        if (!ArchiveService.IsArchive(archivePath))
            return OpResult.Failure(archivePath, "not_an_archive", $"{name} is not an archive Rove can read.");
        ...
        {
            TryDeleteTree(LongPath.ForIo(destination));
            return OpResult.Failure(archivePath, "bad_archive", $"{name} is damaged, or is not really an archive.");
        }
    ...
    )
    {
        ...
        if (ArchiveService.FreeArchive(args.TargetDirectory, stem, args.Format) is not { } destination)
            return CommandResult<OpResult[]>.Fail(
                "already_exists",
                $"There is no free name left for {stem}{ArchiveService.ExtensionFor(args.Format)} here.");
