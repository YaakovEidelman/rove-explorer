...
public readonly record struct ArchivePath(string Archive, string Entry)
{
    ...
    public static bool TryParse(string? path, out ArchivePath location)
    {
        ...
        int bestEnd = -1;
        foreach (string extension in ArchiveService.Extensions)
        {
            if (FindArchiveEnd(display, extension) is not { } end || (bestEnd >= 0 && end >= bestEnd))
                continue;
            bestEnd = end;
        }
        ...
        location = new ArchivePath(display[..bestEnd], Trim(display[bestEnd..]));
        return true;
    }

    private static int? FindArchiveEnd(string display, string extension)
    {
        int from = 0;
