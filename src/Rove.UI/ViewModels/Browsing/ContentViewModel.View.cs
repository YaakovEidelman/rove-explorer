...
public partial class ContentViewModel
{
    ...
    {
        bool hadMarks = DirectoryListing.Items.Any(i => i.IsMarked);
        bool hadSearch = DirectoryListing.InLocalSearch || DirectoryListing.SearchCurrentDirectoryText.Length > 0;
        ClearMarks();
        ClearLocalSearch();
        if (!hadMarks && !hadSearch && _clipboard.HasItems)
        {
            _clipboard.Clear();
            InfoRaised?.Invoke("Cancelled.");
        }
    }
