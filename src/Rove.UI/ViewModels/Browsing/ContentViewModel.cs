...
public partial class ContentViewModel : ViewModelBase
{
    ...
    private bool RefusedInArchive(string verb)
    {
        if (!InArchive)
            return false;
        InfoRaised?.Invoke($"{verb} does not work inside an archive — extract it first.");
        return true;
    }
    ...
    private void RegisterBindings()
    {
        ...
        _registry.Register(CommandDef.ContentSelectAll, SelectAll);
        ...
        _registry.Register(CommandDef.ContentDuplicate, DuplicateItems, canRun: () => !InArchive && !InTrash && !IsAdminView);
        _registry.Register(CommandDef.MoveToHere, MoveToHereItems, canRun: () => !InArchive && !InTrash && !IsAdminView);
        _registry.Register(CommandDef.CopyToHere, CopyToHereItems, canRun: () => !InArchive && !IsAdminView);
        ...
        _registry.Register(CommandDef.CompressItemsTarGz, CompressItemsTarGz,
            canRun: () => !InArchive && !InTrash && !IsAdminView);
