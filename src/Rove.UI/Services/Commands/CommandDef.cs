    ...
    string[]? Keywords = null)
{
    ...
    public static readonly CommandDef ContentSelectAll =
        new("content.select_all", "Select All", CommandKind.User, 20, CommandCategory.File, ["mark all", "select"]);
    ...
    public static readonly CommandDef ContentDuplicate =
        new("content.duplicate", "Duplicate", CommandKind.User, 21, CommandCategory.File, ["copy", "clone"]);
    public static readonly CommandDef MoveToHere =
        new("content.move_to", "Move To…", CommandKind.User, 22, CommandCategory.File,
            ["move", "relocate", "destination", "cut"]);
    public static readonly CommandDef CopyToHere =
        new("content.copy_to", "Copy To…", CommandKind.User, 23, CommandCategory.File,
            ["copy", "duplicate", "destination", "clipboard"]);
    public static readonly CommandDef ExtractArchives =
        new("content.extract", "Extract Archive Here", CommandKind.User, 13, CommandCategory.File,
            ["unzip", "decompress", "archive", "tar", "targz"]);
    ...
    public static readonly CommandDef CompressItemsTarGz =
        new("content.compress_tar_gz", "Compress to tar.gz", CommandKind.User, 15, CommandCategory.File,
            ["tar", "targz", "gzip", "archive"]);
    public static readonly CommandDef ToggleCreateFile =
        new("content.create_file", "New File", CommandKind.User, 16, CommandCategory.File, ["create", "add"]);
    public static readonly CommandDef ToggleCreateFolder =
        new("content.create_folder", "New Folder", CommandKind.User, 17, CommandCategory.File,
    ...
    public static readonly CommandDef UndoLastAction =
        new("content.undo", "Undo Last Action", CommandKind.User, 18, CommandCategory.File, ["revert", "undo"]);
    ...
    public static readonly CommandDef CancelFileOperation =
        new("content.cancel_operation", "Cancel Running File Operation", CommandKind.User, 19, CommandCategory.File,
            ["stop", "abort"]);
