...
One line per command you want to change, command on the left, key on the right:

```json
{
  // Comments and trailing commas are fine.
  "content.move_down": "ctrl+j",
  "content.move_up": "ctrl+k",
  "content.delete": null
}
```

- A command you list **gets** the key you gave it, wherever it already works.
- A command you don't list keeps its default key. You only write down what you change.
- `null` (or `""`) **unbinds** the command completely, so it does nothing at all.
- To put a command on two keys at once, list them together:
  `"content.move_down": ["ctrl+j", "j"]`.

You don't need to know which mode a command lives in — Rove already does, and
moves the command wherever it currently works. See "Commands that live in
more than one place" below for the rare case where that's not enough.

### Giving a command a new key moves it

When you point a command at a key, that command **moves there**: whatever
else used that key by default steps aside, in whichever mode(s) the command
lives in. The example above puts "move down" on `ctrl+j`, so plain `j` no
longer moves down — which is the whole point of moving it.

Only the command you named is affected. Everything you didn't mention keeps
its keys exactly as they were.

### Commands that live in more than one place

Almost every command only works in one mode — `browse`, the file list, for
nearly everything — so you never have to think about modes at all.

A handful of commands (moving up/down, for instance) work in more than one
mode, each with its own default key, because the same physical key can mean
something else there — `j` moves down in the file list, but types the letter
"j" in a search box. Naming a command the normal way changes it everywhere
it already works. To change just one of those places instead, or to give a
brand new key to a command that has no key by default, say which mode you
mean:
...
`"key"` here takes the same shapes as everywhere else: one key, a list of
keys, or `null`/left out to unbind.

### Modes

A mode is whatever surface is currently in front of you. You only need one of
these names for the `"mode"` form above.
...
"Extract Archive Here" without an archive selected — is left out of the list
...
| `content.select_all`        | Mark every item in the folder |
...
| `content.duplicate`         | Duplicate items, next to themselves |
| `content.move_to`           | Move To… (stage a move, same as Cut) |
| `content.copy_to`           | Copy To… (stage a copy, same as Copy) |
| `content.extract`           | Extract an archive here (zip, tar, tar.gz) |
| `content.compress`          | Compress to a zip            |
| `content.compress_tar_gz`   | Compress to a tar.gz         |
...
marked even though nothing in between is. `content.select_all` (`Ctrl+A`)
marks every item in the folder in one go.
...
`content.duplicate` (`Ctrl+D`) copies the highlighted or marked items into the
same folder they're already in, next to themselves, under an auto-generated
name — `report.txt` becomes `report (copy).txt`, then `report (copy 2).txt`,
and so on; a folder named `Notes` becomes `Notes (copy)` the same way. It
does not work inside an archive, the trash, or administrator view.

`content.move_to` (`Shift+X`) and `content.copy_to` (`Shift+Y`) stage the highlighted or marked
items for a move or a copy and exist as their own palette entries ("Move To…"/"Copy To…") so the
two-step "stage it, then walk to where it goes" flow is easy to find by name. While something is
staged, an accent-colored bar appears under the path bar naming what's staged and how many items,
and stays up across navigation so it can't be forgotten mid-move. `content.paste` (`P`) drops it in
whatever folder you're standing in. `content.escape` (`Esc`) cancels the staged transfer, but only
once there are no marks or an open filter left to clear first — so backing out of a filter box with
`Esc` while something is staged doesn't also lose the staged items.

`content.move_to` stages the same way `content.cut` does — a move can only land once, so both
always clear themselves the moment `content.paste` succeeds. `content.copy_to` is a one-shot
version of `content.copy`: it stages a copy that clears itself after the first successful paste,
where plain `content.copy` (`Y`) leaves the clipboard staged so the same items can be pasted into
several folders in a row without copying again each time.
...
refused there, same idea as inside an archive. Looking around, marking, and
