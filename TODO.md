...
6. claiming or giving back the default (Settings row, or the first-run dialog) writes the right config file (confirmed correct), but the already-running xdg-desktop-portal daemon caches portals.conf/<desktop>-portals.conf at its own startup and never re-reads it — so the change has no live effect until the user logs out and back in, even though Rove reports success and the file on disk is correct. `ReloadSessionBus()` (in PortalInstall.cs) only reloads the D-Bus daemon's service-activation files, which is a different cache from the portal daemon's preferred-backend list. Needs either restarting/signaling `xdg-desktop-portal` itself when the claim changes (risky — it's a shared per-session daemon, other apps' open/save dialogs could be mid-use), or telling the user plainly that a logout/login is needed for it to take effect.
