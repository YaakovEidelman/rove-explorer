...
5. Split the oversized files into focused ones (Actions.cs is 1300+ lines, several ViewModels 400+).
6. Group files into folders that match what they do, one type (class, interface, etc.) per file.
7. Redo Open With properly. It is Linux only and needs a real pass on behaviour and tests.
8. Hand-test the published AOT builds on real Wayland and Windows, not only the headless tests.
